...
(function ($) {
		...
		$(element[0]).replaceWith(function () {
			...
			for (var i = 0; i < slides.length; i++) {
				...
				if (slides[i]['data-type'] == "image") { // Template for images
					...
					elements += "<div class='bo-slide " + options.animation + " " + active + "'>\n";
					elements += "<video width='100%'  muted  autoplay  height='500px'>\n";
	...
	function Slider(options, slides) {
		...
		var video = $(this.slides[0]['ref']).find("video")[0];
		var button = $($(this.slides[0]['ref']).find(".play-button")[0]);
		video.play();
		button.css("display", "none");

		this.playing = true;
		var that = this;
		this.resumeVideo = function (n) {debugger
